...
if st.session_state.count == -6:
    ...
    sizi2 = st.empty()
    sizi2.write('リファレンス音源\n何回聞いても問題ありません')
    audio_file=open('Test3_reference_60_ambulance.wav','rb')
    audio_file_test_bytes = audio_file.read()
    sizi2.audio(audio_file_test_bytes, start_time=0)
    # AFC/FS
    sel1 = st.empty()
    ...
    sizi3 = st.empty()
    sizi3.write(' 音源1')
    audio_file=open('Test3_AFC_60_ambulance.wav','rb')
    audio_file_test_bytes = audio_file.read()
    sizi3.audio(audio_file_test_bytes, start_time=0)
    sizi4 = st.empty()
    sizi4.write(' 音源2')
    audio_file=open('Test3_FS_AFC_60_ambulance.wav','rb')
    audio_file_test_bytes = audio_file.read()
    sizi4.audio(audio_file_test_bytes, start_time=0)
    # AFC/B-PEM-AFC
    sel2 = st.empty()
    ...
    sizi6 = st.empty()
    sizi6.write(' 音源2')
    audio_file=open('Test1_B_PEM_AFC_60_ambulance.wav','rb')
    audio_file_test_bytes = audio_file.read()
    sizi6.audio(audio_file_test_bytes, start_time=0)
    # FS-AFC/B-PEM-AFC
    sel3 = st.empty()
